...
export default function NewAdminPage() {
  ...
  const togglePermission = (perm: AdminPermission) => {
    if (role === 'admin') return; // Admin has all permissions by default
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (!res.ok) {
        ...
        throw new Error(data.error || 'Failed to create administrative user');
      }

      router.push('/admin/admins');
    } catch (err: any) {
      console.error('Error creating admin user:', err);
      setError(err.message || 'Error occurred while creating user account.');
    } finally {
      setSubmitting(false);
    }
  };
      ...
      <form onSubmit={handleSubmit} className="space-y-8 font-mono text-xs">
        ...
        <div className="p-6 bg-[#0C0C0C] border border-[#1C1C1C] space-y-6">
          <h3 className="font-display font-bold text-sm text-[#F5F5F5] uppercase border-b border-[#181818] pb-3 flex items-center gap-2">
            ...
            <span>ACCOUNT IDENTITY & ACCESS LEVEL</span>
          </h3>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div className="space-y-2">
              ...
              <input
                ...
                placeholder="e.g. elena@chenabmedia.com"
                className="w-full px-3.5 py-2.5 bg-[#080808] border border-[#222222] text-[#F5F5F5] focus:outline-none focus:border-emerald-500 transition-colors"
              />
            </div>
            ...
            <div className="space-y-2">
              ...
              <select
                ...
                className="w-full px-3.5 py-2.5 bg-[#080808] border border-[#222222] text-[#F5F5F5] focus:outline-none focus:border-emerald-500 uppercase font-bold"
              >
                <option value="executive">EXECUTIVE (Granular Permissions)</option>
                <option value="admin">ADMINISTRATOR (Full System Access)</option>
              </select>
              <p className="text-[10px] text-[#666666]">
                Note: Artist role accounts cannot be created from this portal.
              </p>
            </div>
