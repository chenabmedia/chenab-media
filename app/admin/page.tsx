...
export default function AdminOverviewPage() {
  ...
  const fetchDashboardData = async () => {
    ...
    try {
      ...
      // 4. Fetch Journal Articles Count
      let jCount = 0;
      try {
        const jSnap = await getDocs(collection(db, 'journal'));
        jCount = jSnap.size;
      } catch (e) {
        console.warn('Could not fetch journal count:', e);
      }

      // 5. Fetch Contact Submissions Count
      let mCount = 0;
      try {
      ...
      if (aCount === 0 && rCount === 0 && dCount === 0 && mCount === 0 && jCount === 0) {
        setStats({
          artistsCount: 12,
          releasesCount: 28,
          journalCount: 4,
  ...
  return (
    <div className="space-y-8">
      ...
      <div className="border-b border-[#1A1A1A] pb-6 flex flex-col md:flex-row md:items-center justify-between gap-4">
        ...
        <div className="flex items-center gap-3">
          ...
          <Link
            ...
            className="px-4 py-2.5 bg-[#141414] hover:bg-[#1E1E1E] border border-[#333333] text-[#F5F5F5] transition-colors font-mono text-xs uppercase font-bold flex items-center gap-2"
          >
            <BookOpen size={14} />
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-4">
        ...
        <div className="p-5 bg-[#0C0C0C] border border-[#1C1C1C] space-y-3 relative overflow-hidden group hover:border-[#333333] transition-colors">
          <div className="flex items-center justify-between text-[#888888]">
            <span className="font-mono text-xs uppercase tracking-wider">JOURNAL POSTS</span>
            <BookOpen size={16} className="text-[#666666] group-hover:text-purple-400 transition-colors" />
          </div>
          <div className="flex items-baseline justify-between">
            <span className="font-display font-extrabold text-3xl sm:text-4xl text-[#F5F5F5]">
              {loading ? <Loader2 size={24} className="animate-spin text-[#666666]" /> : stats.journalCount}
            </span>
            <Link
              href="/admin/journal"
              className="font-mono text-[11px] text-purple-400 hover:underline flex items-center gap-1"
            >
              <span>CMS</span>
              <ChevronRight size={12} />
            </Link>
