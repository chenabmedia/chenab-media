...
function LoginContent() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (mode === 'signin') {
        ...
        showToast('Authentication successful. Loading portal...', 'success');
        ...
        const notice = 'Security key reset link has been dispatched to your email address.';
        setSuccessNotice(notice);
        showToast(notice, 'success');
      }
    } catch (err: any) {
      const errMsg = err.message || 'Operation failed. Please try again.';
  ...
  if (user && !userProfile && !isSuperAdminEmail(user.email)) {
    return (
      <div className="max-w-md mx-auto px-4 sm:px-6 py-10 sm:py-16 space-y-6 sm:space-y-8 font-mono">
        <div className="text-center space-y-3">
          ...
          <h1 className="font-display font-extrabold text-2xl sm:text-3xl text-[#F5F5F5] tracking-tight uppercase">
        ...
        <div className="border border-[#1A1A1A] bg-[#0C0C0C] p-4 sm:p-6 space-y-5 sm:space-y-6 text-xs">
          <p className="text-[#A0A0A0] leading-relaxed break-all">
          ...
          <button
            ...
            className="w-full py-3.5 min-h-[44px] bg-[#111111] border border-[#222222] text-red-400 font-bold uppercase tracking-widest hover:bg-red-950/20 hover:border-red-900/50 transition-all flex items-center justify-center gap-2 disabled:opacity-50"
          >
            {isSigningOut ? (
      ...
      <div className="max-w-md mx-auto px-4 sm:px-6 py-10 sm:py-16 space-y-6 sm:space-y-8 font-mono">
        ...
        <div className="border border-[#1A1A1A] bg-[#0C0C0C] p-4 sm:p-6 space-y-5 sm:space-y-6">
          ...
          <div className="space-y-3">
            <Link
              href={portalLink}
              className="w-full py-3.5 min-h-[44px] bg-[#F5F5F5] text-[#080808] text-xs font-bold uppercase tracking-widest hover:bg-white transition-all flex items-center justify-center gap-2"
            >
            ...
            <button
              onClick={handleSignOut}
              disabled={isSigningOut}
              className="w-full py-3.5 min-h-[44px] bg-[#111111] border border-[#222222] text-red-400 text-xs font-bold uppercase tracking-widest hover:bg-red-950/20 hover:border-red-900/50 transition-all flex items-center justify-center gap-2 disabled:opacity-50"
            >
              {isSigningOut ? (
                <Loader2 size={14} className="animate-spin text-red-400" />
              ) : (
              ...
              <span>{isSigningOut ? 'TERMINATING SESSION...' : 'TERMINATE SESSION / SIGN OUT'}</span>
            </button>
          </div>
        </div>
      </div>
  ...
  return (
    <div className="max-w-md mx-auto px-4 sm:px-6 py-10 sm:py-16 space-y-6 sm:space-y-8 font-mono">
      <div className="text-center space-y-3">
        ...
        <h1 className="font-display font-extrabold text-[clamp(1.75rem,5vw,2.25rem)] text-[#F5F5F5] tracking-tight uppercase leading-tight">
          CHENAB PORTAL
        </h1>
        <p className="text-xs text-[#888888]">
      ...
      <div className="border border-[#1A1A1A] bg-[#0C0C0C] p-4 sm:p-6 space-y-5 sm:space-y-6">
        <form onSubmit={handleSubmit} className="space-y-4 sm:space-y-5">
          <div className="space-y-2 text-xs">
            ...
            <div className="relative">
              <input
                ...
                className="w-full bg-[#111111] border border-[#222222] p-3 pl-10 min-h-[44px] text-[#F5F5F5] placeholder-[#555555] focus:outline-none focus:border-[#555555]"
              />
              ...
              <div className="flex items-center justify-between">
                ...
                {mode === 'signin' && (
                  <button
                    ...
                    className="text-[11px] text-[#888888] hover:text-[#CCCCCC] underline py-1 min-h-[32px] inline-flex items-center"
                  >
                    FORGOT KEY?
                  </button>
                )}
          ...
          <button
            ...
            className="w-full py-3.5 min-h-[48px] bg-[#F5F5F5] text-[#080808] text-xs font-bold uppercase tracking-widest hover:bg-white transition-all flex items-center justify-center gap-2 disabled:opacity-50"
          >
            {isSubmitting ? (
              <>
          ...
          {mode === 'forgot' && (
            <button
              ...
              className="w-full text-center text-xs text-[#888888] hover:text-[#F5F5F5] underline pt-2 min-h-[44px] flex items-center justify-center"
            >
              RETURN TO SIGN IN
            </button>
          )}
