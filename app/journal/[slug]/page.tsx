...
export default function JournalDetailPage({ params }: PageProps) {
  ...
  const [post, setPost] = useState<JournalPost | null>(() => (slug ? getJournalPostBySlug(slug) || null : null));
  ...
  useEffect(() => {
    async function loadPost() {
      ...
      try {
        ...
        console.warn('Failed to load dynamic post from API:', err);
      } finally {
        setLoading(false);
      }
    }
  ...
  if (loading && !post) {
    return (
      <div className="max-w-4xl mx-auto px-6 py-20 sm:py-24 text-center space-y-6">
        <div className="font-mono text-xs text-[#888888] animate-pulse">LOADING JOURNAL ENTRY...</div>
      </div>
    );
  }
