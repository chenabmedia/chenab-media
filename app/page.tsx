...
import { Artist, Release, JournalPost } from '@/types';
...
export default function HomePage() {
  ...
  const [posts, setPosts] = useState<JournalPost[]>(() => getAllJournalPosts());
  ...
  useEffect(() => {
    async function loadPublicLiveCatalog() {
      try {
        const [relRes, artRes, jnlRes] = await Promise.all([
          fetch('/api/releases'),
          fetch('/api/artists'),
          fetch('/api/journal'),
        ]);
        ...
        if (jnlRes.ok) {
          const jnlData = await jnlRes.json();
          if (jnlData.posts && jnlData.posts.length > 0) {
            setPosts(jnlData.posts);
          }
        }
      } catch (err) {
