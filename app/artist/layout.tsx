  ...
  return (
    <ProtectedRoute allowedRoles={['artist', 'executive', 'admin']}>
      <div className="min-h-screen bg-[#080808] text-[#F5F5F5] flex flex-col font-sans">
        {/* Top Header Bar */}
        <header className="h-14 border-b border-[#1C1C1C] bg-[#0A0A0A] px-6 flex items-center justify-between font-mono text-xs z-20">
          ...
          <div className="flex items-center gap-3">
            <Link
              href="/"
              className="px-3 py-1.5 border border-[#222222] bg-[#111111] hover:border-[#444444] text-[#CCCCCC] hover:text-white transition-colors uppercase flex items-center gap-2 text-[11px]"
            >
              <ArrowLeft size={13} />
            ...
            <button
              onClick={() => signOut()}
              className="px-3 py-1.5 border border-red-900/40 bg-red-950/30 text-red-300 hover:bg-red-900/40 transition-colors uppercase font-bold flex items-center gap-1.5 text-[11px]"
            >
              <LogOut size={13} />
              <span>SIGN OUT</span>
            </button>
