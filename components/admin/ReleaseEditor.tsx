...
export function ReleaseEditor({ initialRelease, isEditMode = false }: ReleaseEditorProps) {
  ...
  const handleSave = async (targetStatus?: string) => {
    ...
    if (finalStatus === 'PUBLISHED' && !canPublish) {
      setError('Cannot publish: Please complete all mandatory metadata requirements in the checklist.');
      return;
    }
    ...
    try {
      ...
      setSuccessMessage(
        `Release successfully ${finalStatus === 'PUBLISHED' ? 'published' : 'saved'}!`
      );
      setTimeout(() => {
        router.push('/admin/releases');
      }, 1200);
    } catch (err: any) {
      setError(err.message || 'Error occurred while saving release');
    } finally {
      setSaving(false);
    }
  };
    ...
    <div className="space-y-8">
      ...
      <div className="bg-[#0C0C0C] border border-[#1C1C1C] p-6 flex flex-col md:flex-row md:items-center justify-between gap-6 shadow-xl">
        ...
        <div className="flex flex-wrap items-center gap-3 font-mono text-xs">
          ...
          >
            <Send size={14} />
            <span>{status === 'PUBLISHED' ? 'UPDATE PUBLISHED' : 'PUBLISH RELEASE'}</span>
          </button>
          ...
          {status === 'PUBLISHED' && (
            <button
              ...
              disabled={saving}
              className="px-3 py-2 bg-purple-950/20 border border-purple-900/40 text-purple-300 font-mono text-[11px] hover:bg-purple-900/30"
            >
              UNPUBLISH
            </button>
          )}
