  ...
  const handleArchive = async (id: string, title: string) => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        showToast(`Failed to archive release.`, 'error');
      }
    } catch (e) {
      console.error(e);
      showToast(`Error archiving release.`, 'error');
    } finally {
      setArchivingId(null);
    }
  };
      ...
      <div className="border border-[#1C1C1C] bg-[#0C0C0C]">
        {loading ? (
          <div className="p-6 space-y-4">
            <div className="flex items-center gap-3 text-emerald-400 text-xs font-mono mb-4">
              <Loader2 size={14} className="animate-spin" />
              <span>SYNCING CATALOGUE RELEASES...</span>
            </div>
            {[1, 2, 3, 4, 5].map((idx) => (
              <div
                key={idx}
                className="h-14 bg-[#111111] animate-pulse border border-[#1C1C1C] rounded-none flex items-center justify-between px-4"
              >
                <div className="flex items-center gap-3">
                  <div className="w-10 h-10 bg-[#1A1A1A]" />
                  <div className="space-y-1.5">
                    <div className="w-36 h-3 bg-[#222222]" />
                    <div className="w-24 h-2.5 bg-[#181818]" />
                  </div>
                </div>
                <div className="w-16 h-4 bg-[#1A1A1A]" />
              </div>
            ))}
          </div>
        ) : filtered.length === 0 ? (
            ...
            <table className="w-full text-left border-collapse">
              ...
              <tbody className="divide-y divide-[#181818]">
                {filtered.map((rel) => {
                  ...
                  const isArchiving = archivingId === rel.id;
                  return (
                      ...
                      <td className="p-4 text-right">
                        <div className="flex items-center justify-end gap-2">
                          ...
                          <button
                            ...
                            className="p-1.5 border border-red-900/30 text-red-400 hover:bg-red-950/30 disabled:opacity-50"
